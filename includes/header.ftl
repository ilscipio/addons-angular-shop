...
<#macro logoMenu hasLink=true isSmall=false>
    ...
    <#if headerImageUrl??>
        ...
        <#else>
        <a href="<@ofbizUrl>${logoLinkURL!""}</@ofbizUrl>" class="navbar-brand"><img alt="${layoutSettings.companyName}" src="<@ofbizContentUrl>/images/cato/<#if isSmall>cato-logo-small.png<#else>cato-logo.svg</#if></@ofbizContentUrl>"/></a>
    </#if>
</#macro>
